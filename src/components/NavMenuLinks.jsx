...
const NavMenuLinks = () => {
  ...
  return (
    <div className="flex justify-center md:justify-end text-[#324B4C] dark:text-white font-medium">
      <Link to={location.pathname === '/about' ? '/' : '/about'}>
        <div className="flex items-center mr-2">
          {location.pathname === '/about' ? (
            <FiHome className="text-[#324B4C] dark:text-white inline-block mr-2 md:m-1" />
          ) : (
            <FiUser className="text-[#324B4C] dark:text-white inline-block mr-2 md:m-1" />
          )}
